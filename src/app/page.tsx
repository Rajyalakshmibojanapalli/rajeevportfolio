    ...
    <div className="relative min-h-screen w-full bg-white py-10 flex flex-col items-center justify-start">
      ...
      <div className="group flex flex-col md:flex-row w-full min-h-[600px] sm:min-h-[700px] lg:h-[700] lg:-mb-2 xl:mb-0 xl:h-[846px] px-4 sm:px-6 md:px-8 gap-6 sm:gap-8 md:gap-[71px] items-center justify-center mt-6">
        <div className="hidden lg:flex flex-col w-[328px] h-[138px] items-start justify-start transition-transform duration-300 ease-in-out group-hover:-translate-y-[250px]">
          ...
          <p className="text-[#344054] text-[16px] font-medium leading-snug">
            I’m Prasanna Rajeev Putta, a results-driven Senior Digital Marketing Specialist with 5+
            years of experience across performance marketing, growth strategy, and social media
            management. My passion lies in scaling businesses through data-driven campaigns,
            storytelling, and creative digital solutions. Beyond marketing,
            <br />
            I’m a passionate rider and content
            creator, blending my love for adventure with impactful digital storytelling.
          </p>
        </div>
        ...
        <div className="relative w-full flex flex-col items-center justify-center">
          <div className="flex w-full max-w-[952px] flex-col items-center justify-center  transition-all duration-300 ease-in-out group-hover:translate-y-[280px] group-hover:opacity-0 px-4 sm:px-6">
            ...
            <div className="flex flex-col sm:flex-row sm:gap-2 items-center sm:items-end mb-1 mt-6 sm:mt-8">
              ...
              <div className="flex items-end">
                <CustomeText title="Prasanna Rajeev " className="text-[#FD853A] font-semibold text-4xl sm:text-5xl md:text-7xl xl:text-[88px]" />
                <CustomeText title="," className="text-[#171717] font-semibold text-4xl sm:text-5xl md:text-7xl xl:text-[88px]" />
              </div>
            </div>
            <CustomeText title="Digital Marketing" className="text-[#171717] font-semibold text-4xl sm:text-5xl md:text-7xl xl:text-[88px] text-center" />
          </div>
          ...
          <div className="relative w-full max-w-[952px] aspect-[3/2] flex flex-col items-center justify-center -translate-y-[10%] sm:-translate-y-[15%] md:-translate-y-[20%] mx-auto px-4">
            ...
            <Image
              ...
              width={200}
              height={136}
              className="relative z-20 w-[480px] h-auto object-contain mt-5"
              priority
            />
      ...
      <div className="relative flex flex-col w-full min-h-[878px] gap-16 sm:gap-[96px] items-center px-4 sm:px-6 lg:px-[71px] py-16 sm:py-[116px] bg-[#171717] rounded-[30px] sm:rounded-[50px] overflow-hidden -translate-y-8 sm:-translate-y-0 md:-translate-y-15 lg:translate-y-0 -mt-14">
        ...
        <div className="w-full flex flex-col lg:flex-row items-start justify-between gap-6 relative z-10">
          ...
          <p className="w-full lg:w-[578px] font-medium text-base sm:text-lg lg:text-[20px] text-white">
            I help businesses grow through data-driven digital marketing — from strategy and performance campaigns to SEO, social media, and brand building
          </p>
        </div>
      ...
      <div className="w-full bg-white py-16 lg:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Section Title */}
          <div className="flex flex-wrap items-baseline justify-center lg:justify-start mb-16">
            <CustomeText title="My" className="font-medium text-4xl sm:text-5xl lg:text-6xl text-[#344054] mr-3" />
            <CustomeText title="Work" className="font-medium text-4xl sm:text-5xl lg:text-6xl text-[#FD853A] mr-3" />
            <CustomeText title="Experience" className="font-medium text-4xl sm:text-5xl lg:text-6xl text-[#FD853A]" />
          </div>
          ...
          <div className="lg:hidden">
            {experiences.map((exp, index) => (
              <div key={index} className="relative pl-8 pb-12 last:pb-0 before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-0 before:w-[2px] before:border-l-2 before:border-dashed before:border-[#1D2939] last:before:border-transparent">
                <div className="relative">
                  {/* Timeline Dot */}
                  <div className="absolute -left-8 top-2">
                    <div className="w-6 h-6 rounded-full border-2 border-dashed border-[#1D2939] bg-white flex items-center justify-center">
                      <div className={`w-4 h-4 rounded-full ${exp.dotColor}`}></div>
                    </div>
                  </div>

                  {/* Content */}
                  <div className="space-y-2">
                    <CustomeText title={exp.company} className="font-semibold text-[20px] sm:text-[24px] text-[#1D2939]" />
                    <CustomeText title={exp.duration} className="text-[14px] sm:text-[16px] text-[#98A2B3]" />
                    <CustomeText title={exp.role} className="font-semibold text-[18px] sm:text-[20px] text-[#1D2939]" />
                    {exp.desc && (
                      <CustomeText title={exp.desc} className="text-[14px] sm:text-[16px] text-[#98A2B3] leading-relaxed" />
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="hidden lg:block">
            <div className="relative">
              ...
              <div className="absolute left-1/2 transform -translate-x-1/2 top-8 bottom-8 w-[2px] border-l-2 border-dashed border-[#1D2939]"></div>

              {experiences.map((exp, index) => (
                <div key={index} className={`flex items-start ${index !== 0 ? 'mt-32' : ''}`}>
                  {/* Left Column */}
                  <div className="w-[45%] pr-10 text-right">
                    {index % 2 === 0 ? (
                      <>
                        <CustomeText title={exp.company} className="font-semibold text-[28px] xl:text-[32px] text-[#1D2939] mb-2" />
                        <CustomeText title={exp.duration} className="text-lg xl:text-xl text-[#98A2B3]" />
                      </>
                    ) : (
                      <>
                        <CustomeText title={exp.role} className="font-semibold text-[28px] xl:text-[32px] text-[#1D2939] mb-2" />
                        {exp.desc && (
                          <CustomeText title={exp.desc} className="text-lg xl:text-xl text-[#98A2B3] leading-relaxed" />
                        )}
                      </>
                    )}
                  </div>
                  ...
                  <div className="flex justify-center w-[10%]">
                    <div className="relative w-10 h-10 rounded-full border-2 border-dashed border-[#1D2939] bg-white flex items-center justify-center z-10">
                      <div className={`w-7 h-7 rounded-full ${exp.dotColor}`}></div>
                    </div>
                  </div>
      ...
      <div className="w-full flex flex-col lg:flex-row items-center justify-between px-6 lg:px-[71px] py-20 lg:py-[122px] bg-[#F2F4F7] rounded-[32px] lg:rounded-[50px] gap-12 lg:gap-[96px]">
        ...
        <div className="w-full max-w-xl flex flex-col items-start gap-8">
          ...
          <p className="text-[#98A2B3] text-base sm:text-lg lg:text-xl leading-relaxed max-w-md">
            Because I bring the perfect balance of creative storytelling and analytical precision. I’ve
            scaled multiple brands through digital marketing, delivering measurable ROI and consistent
            growth. I thrive in fast-paced environments, adapt quickly to evolving trends, and bring
            multi-industry expertise (education, healthcare, retail, interiors, and more). On top of that, I
            combine my leadership experience with hands-on skills, ensuring strategy and execution go
            hand in hand. </p>
      ...
      <div className="relative flex flex-col w-full min-h-[900px] items-center px-4 sm:px-6 lg:px-[71px] py-12 sm:py-[96px]  gap-12 bg-[#171717] rounded-[30px] sm:rounded-[40px] lg:rounded-[50px] overflow-hidden">
        ...
        <div className="flex flex-col w-full max-w-[1299px] items-center gap-4 z-10 px-2">
          ...
          <p className="w-full max-w-[742px] text-[16px] sm:text-[18px] lg:text-[20px] text-[#F9FAFB] text-center leading-[1.6] px-2">
            I help businesses grow through data-driven digital marketing — from strategy and performance campaigns to SEO, social media, and brand building </p>
        </div>
