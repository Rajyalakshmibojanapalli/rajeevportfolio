...
export default function ReviewCard({ name, role, rating, text }: Review) {
  return (
    <div className="flex flex-col w-full h-auto min-h-[200px] sm:min-h-[220px] lg:min-h-[265px] items-start gap-3 sm:gap-3.5 lg:gap-4 p-4 sm:p-5 lg:p-6 rounded-xl sm:rounded-2xl lg:rounded-3xl bg-[#FFFFFF]/14 backdrop-blur-[14px] opacity-80 hover:opacity-100 transition-opacity duration-300 border border-white/10 hover:border-white/20">
      {/* Header with Avatar and Info */}
      <div className="w-full flex flex-col sm:flex-row gap-3 sm:gap-4 items-start sm:items-center">
        {/* Avatar with background */}
        <div className="w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 flex-shrink-0 flex items-center justify-center bg-gradient-to-br from-[#FD853A]/20 to-[#FD853A]/5 rounded-full border border-white/20">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
      ...
      {/* Rating Section - Enhanced with empty stars */}
      <div className="w-full flex items-center gap-2 sm:gap-3 lg:gap-4">
        <div className="flex gap-1 sm:gap-1.5">
          {[...Array(5)].map((_, i) => (
            <Star 
              key={i} 
              size={16} 
              className="sm:w-5 sm:h-5 lg:w-6 lg:h-6 xl:w-7 xl:h-7 transition-all duration-300" 
              fill={i < rating ? "#FD853A" : "transparent"} 
              stroke={i < rating ? "#FD853A" : "#FCFCFD40"} 
            />
          ))}
        </div>
        <span className="text-lg sm:text-xl lg:text-2xl xl:text-3xl font-semibold text-[#FCFCFD]">{rating}</span>
